...
> Note: Normally you would ensure that you always `pull` before `pushing` so that any newer changes on the GitHub copy of the repo are compared and incorporated. GitHub has a handy feature where it regularly checks for updates on the GitHub repo and will prompt you to `pull` when needed.
